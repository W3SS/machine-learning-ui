...
public class GenerateTests : MonoBehaviour
{
	...
	public void GenerateMultiClassHardModel() {
		...
		for (float areaToPopulateX = -10; areaToPopulateX < 10; areaToPopulateX = areaToPopulateX+ 5) {
			for (float areaToPopulateZ = -10; areaToPopulateZ < 10; areaToPopulateZ = areaToPopulateZ + 5) {
				for (var i = 0; i < 10; i++) {
					...
					if (indexColor == 0) {
						blueSamples.Add (Instantiate (bluePrefab, new Vector3 (xRdm, 0f, zRdm), Quaternion.identity));
					} else if (indexColor == 1) {
						redSamples.Add (Instantiate (redPrefab, new Vector3 (xRdm, 0f, zRdm), Quaternion.identity));
					} else if (indexColor == 2) {
						greenSamples.Add (Instantiate (greenPrefab, new Vector3 (xRdm, 0f, zRdm), Quaternion.identity));
					}
				}
				indexColor++;
			}
